...
class ALS(object):
    ...
    def _process_data(self, X):
        ...
        # Process user ids.
        self.user_ids = tuple((set(map(lambda x: x[1], X))))
        self.user_ids_dict = dict(map(lambda x: x[::-1],
                                      enumerate(self.user_ids)))

        # Process item ids.
        self.item_ids = tuple((set(map(lambda x: x[0], X))))
        ...
        self.ratings_T = defaultdict(lambda: defaultdict(int))
        for row in X:
            item_id, user_id, rating = row
            self.ratings[user_id][item_id] = rating
            self.ratings_T[item_id][user_id] = rating

        # Result validation.
        err_msg = "Length of user_ids %d and ratings %d not match!" % (
            len(self.user_ids), len(self.ratings))
        assert len(self.user_ids) == len(self.ratings), err_msg

        err_msg = "Length of item_ids %d and ratings_T %d not match!" % (
            len(self.item_ids), len(self.ratings_T))
        assert len(self.item_ids) == len(self.ratings_T), err_msg
        return self.ratings, self.ratings_T
    ...
    def _users_mul_ratings(self, users, ratings_T):
        ...
        def f(users_row, item_id):
            user_ids = iter(self.ratings_T[item_id].keys())
            scores = iter(self.ratings_T[item_id].values())
    ...
    def _predict(self, user_id, item_id):
        ...
        try:
           ...
           # 代表item_id的向量
           try:
              item_col=self.item_matrix.col(self.item_ids_dict[item_id])
           except KeyError as e:
              pass
           else:
           # 算出得分
              items_score = users_col.mat_mul(item_col).data[0]
